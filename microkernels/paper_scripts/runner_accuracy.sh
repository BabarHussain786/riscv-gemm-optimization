...
PROJECT_ROOT="${PAPER_PROJECT_ROOT:?Run through paper_campaign.py}"
SCRIPT_DIR="${PAPER_CASE_DIR:?Missing isolated case directory}"
HARNESS="${PROJECT_ROOT}/RVV_IME_GEMM_ACCURACY_VALIDATION/ime_rvv_fallback_diff_histogram_check.c"
STAMP="$(date +%Y%m%d_%H%M%S)"
RESULT_DIR="${SCRIPT_DIR}/int8_ime_vs_rvv_accuracy_once_${MODE}_${M}_${STAMP}"
...
usage()
{
    cat <<'EOF'
Usage:
  bash run_int8_ime_vs_rvv_accuracy_once.sh MODE [RUNS]
...
log "Logs=${LOG_DIR}"
if [ "${build_failed_count}" -ne 0 ] || [ "${run_failed_count}" -ne 0 ] || [ "${ok_count}" -eq 0 ]; then
    exit 1
fi
