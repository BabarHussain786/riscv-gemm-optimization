...
SCRIPT_DIR="${PAPER_PROJECT_ROOT:?Run through paper_campaign.py}"
M="${M:-1024}"
N="${N:-1024}"
K="${K:-1024}"
RUNS="${RUNS:-6}"
...
run_family() {
  ...
  for dir in "${dirs[@]}"; do
    ...
    local make_args=("CC=${CC:-gcc}")
    kernel="$(basename "${dir}")"
    [[ "${kernel}" == "${PAPER_KERNEL_FILTER:?Missing kernel filter}" ]] || continue
...
if [ "${CAMPAIGN_STATUS}" != "OK" ] || [ "${BOUNDARY_VALIDATION_FAILED_COUNT}" -ne 0 ] || [ "${OK_RUN_COUNT}" -eq 0 ]; then
  exit 1
fi
