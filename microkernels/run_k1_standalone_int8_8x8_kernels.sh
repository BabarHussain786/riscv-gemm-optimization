...
# STANDALONE 8x8 KERNEL BENCHMARK ROADMAP
# Step 1 -> Build one direct RVV or IME micro-kernel.
# Step 2 -> Validate the selected matrix size once (default: 1024x1024x1024).
...
OUT_DIR="${OUT_DIR:-${SCRIPT_DIR}/k1_standalone_int8_8x8_results_${M}_${STAMP}}"
...
RVV_ROOT="${SCRIPT_DIR}/GEMM_RVV_FP32_INT8_8x8_Baseline/RVV_IGEMM_INT8_I8I32_8x8"
IME_ROOT="${SCRIPT_DIR}/IME_NATIVE_KERNELS/IME_GEMM_INT8_I8I32_8x8_NATIVE"
...
print_summary()
{
    ...
    log "GEMM=${M}x${N}x${K} | OpenMP=NO | LMUL=1 | tile=8x8 | IPC=${COLLECT_PERF}"
    log "-------------------------------------------------------------------------------------"
...
for unroll in 1 2 4 8; do
    rvv_dir="igemm_kernel_8x8_zvl256b_lmul1_unroll${unroll}"
    rvv_kernel="${rvv_dir}_i8i32"
    ime_kernel="ime_kernel_8x8_zvl256b_lmul1_unroll${unroll}"
