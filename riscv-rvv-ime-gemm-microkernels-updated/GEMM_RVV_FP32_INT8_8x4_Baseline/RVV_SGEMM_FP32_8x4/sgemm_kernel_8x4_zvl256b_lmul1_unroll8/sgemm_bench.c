    ...
    for (BLASLONG n = 0; n < N; ++n) {
        for (BLASLONG m = 0; m < M; ++m) {
            ...
            for (BLASLONG k = 0; k < K; ++k) {
                ...
                const size_t bi = packed_offset(n, N, 4, k, K);
                              ...
                              size_t count, double *max_abs_error)
{
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        const double tolerance = 1e-5 * (1.0 + fabs(r));
...
int main(int argc, char **argv)
{
    ...
    const int rc = CNAME(M, N, K, 1.0f, A, B, C, M);
    ...
    if (validate) {
        ...
        compute_reference(M, N, K, 1.0f, A, B, C_initial, reference);
