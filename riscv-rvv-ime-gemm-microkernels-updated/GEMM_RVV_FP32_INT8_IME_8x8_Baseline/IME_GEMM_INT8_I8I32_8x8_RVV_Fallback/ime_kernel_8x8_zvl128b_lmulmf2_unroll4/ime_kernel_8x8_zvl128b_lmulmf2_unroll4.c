...
*/
/*
 * Kernel dataflow summary: ime_kernel_8x8_zvl128b_lmulmf2_unroll4
 ...
 * ---------------------
 * Purpose: IME INT8 GEMM front-end for the 8x8 micro-tile family.
...
static void boundary_cleanup_gemm_block_range(BLASLONG rows, BLASLONG cols,
...
static int boundary_cleanup_gemm_full_prepacked(BLASLONG M, BLASLONG N, BLASLONG K,
                                      int32_t alpha,
                                      const int8_t *A,
                                      const int8_t *B,
                                      int32_t *C,
    ...
    while (n_top < N) {
        ...
        while (m_top < M) {
            ...
            boundary_cleanup_gemm_block_range(rows, cols, K, 0, K, alpha,
                                    &A[m_top * K],
                                    &B[n_top * K],
                                    &C[n_top * ldc + m_top],
                                    ldc);
...
#endif
        if (K_main != K) {
            boundary_cleanup_gemm_block_range(8, 4, K, K_main, K, alpha, A_blk, B4_blk, C_blk, ldc);
        }
        return;
    }

    boundary_cleanup_gemm_block_range(8, 4, K, 0, K, alpha, A_blk, B4_blk, C_blk, ldc);
}
...
static int ime_gemm_s8s8_prepacked(BLASLONG M, BLASLONG N, BLASLONG K,
    ...
    for (BLASLONG j = 0; j < N / 8; ++j) {
        ...
        if (M & 4) {
            boundary_cleanup_gemm_block_range(4, 8, K, 0, K, alpha,
        ...
        if (M & 2) {
            boundary_cleanup_gemm_block_range(2, 8, K, 0, K, alpha,
                                    pack_A_panel(A, m_top, K),
                                    &B[n_top * K],
                                    &C[n_top * ldc + m_top],
                                    ldc);
        ...
        if (M & 1) {
            boundary_cleanup_gemm_block_range(1, 8, K, 0, K, alpha,
                                    pack_A_panel(A, m_top, K),
                                    &B[n_top * K],
                                    &C[n_top * ldc + m_top],
                                    ldc);
    ...
    if (N & 4) {
        ...
        if (M & 4) {
            boundary_cleanup_gemm_block_range(4, 4, K, 0, K, alpha,
                                    pack_A_panel(A, m_top, K),
                                    &B[n_top * K],
                                    &C[n_top * ldc + m_top],
                                    ldc);
        ...
        if (M & 2) {
            boundary_cleanup_gemm_block_range(2, 4, K, 0, K, alpha,
                                    pack_A_panel(A, m_top, K),
                                    &B[n_top * K],
                                    &C[n_top * ldc + m_top],
                                    ldc);
        ...
        if (M & 1) {
            boundary_cleanup_gemm_block_range(1, 4, K, 0, K, alpha,
                                    pack_A_panel(A, m_top, K),
                                    &B[n_top * K],
                                    &C[n_top * ldc + m_top],
                                    ldc);
    ...
    if (N & 2) {
        ...
        for (BLASLONG i = 0; i < M / 8; ++i) {
            boundary_cleanup_gemm_block_range(8, 2, K, 0, K, alpha,
                                    pack_A_panel(A, m_top, K),
                                    &B[n_top * K],
                                    &C[n_top * ldc + m_top],
                                    ldc);
        ...
        if (M & 4) {
            boundary_cleanup_gemm_block_range(4, 2, K, 0, K, alpha,
                                    pack_A_panel(A, m_top, K),
                                    &B[n_top * K],
                                    &C[n_top * ldc + m_top],
                                    ldc);
        ...
        if (M & 2) {
            boundary_cleanup_gemm_block_range(2, 2, K, 0, K, alpha,
                                    pack_A_panel(A, m_top, K),
                                    &B[n_top * K],
                                    &C[n_top * ldc + m_top],
                                    ldc);
        ...
        if (M & 1) {
            boundary_cleanup_gemm_block_range(1, 2, K, 0, K, alpha,
                                    pack_A_panel(A, m_top, K),
                                    &B[n_top * K],
                                    &C[n_top * ldc + m_top],
                                    ldc);
    ...
    if (N & 1) {
        ...
        for (BLASLONG i = 0; i < M / 8; ++i) {
            boundary_cleanup_gemm_block_range(8, 1, K, 0, K, alpha,
                                    pack_A_panel(A, m_top, K),
                                    &B[n_top * K],
                                    &C[n_top * ldc + m_top],
                                    ldc);
        ...
        if (M & 4) {
            boundary_cleanup_gemm_block_range(4, 1, K, 0, K, alpha,
                                    pack_A_panel(A, m_top, K),
                                    &B[n_top * K],
                                    &C[n_top * ldc + m_top],
                                    ldc);
        ...
        if (M & 2) {
            boundary_cleanup_gemm_block_range(2, 1, K, 0, K, alpha,
                                    pack_A_panel(A, m_top, K),
                                    &B[n_top * K],
                                    &C[n_top * ldc + m_top],
                                    ldc);
        ...
        if (M & 1) {
            boundary_cleanup_gemm_block_range(1, 1, K, 0, K, alpha,
                                    pack_A_panel(A, m_top, K),
                                    &B[n_top * K],
                                    &C[n_top * ldc + m_top],
                                    ldc);
