 ...
 *
 * SpacemiT IME INT8 GEMM microkernel: ime_kernel_8x4_zvl256b_lmul1_unroll1
 *
 * Child-friendly view:
 ...
 *
 * Software tile:
 *   8x4. The driver composes complete native hardware tiles internally.
 *
 * Documented LMUL:
 *   The current A60 and A100 profiles support LMUL=1 for IME instructions.
 *
 * Portability:
...
int igemm_kernel_8x4_zvl256b_lmul1_unroll1_i8i32(BLASLONG M, BLASLONG N, BLASLONG K,
                             ...
                             int32_t *C, BLASLONG ldc)
{
    ...
    rc = igemm_kernel_8x4_zvl256b_lmul1_unroll1_i8i32(M, N, K, alpha, A_pack, B_pack, C, ldc);
    free(A_pack);
    free(B_pack);
    if (rc == 0) return 0;
    return scalar_gemm_full(M, N, K, alpha, A, B, C, ldc);
...
int ime_kernel_8x4_zvl256b_lmul1_unroll1(BLASLONG M, BLASLONG N, BLASLONG K,
              int32_t alpha, const int8_t *A, const int8_t *B,
              int32_t *C, BLASLONG ldc)
{
