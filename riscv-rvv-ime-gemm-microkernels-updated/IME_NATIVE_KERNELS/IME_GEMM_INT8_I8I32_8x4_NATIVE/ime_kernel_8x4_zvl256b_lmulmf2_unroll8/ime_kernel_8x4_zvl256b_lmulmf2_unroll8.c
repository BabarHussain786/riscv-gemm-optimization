...
int igemm_kernel_8x4_zvl256b_lmulmf2_unroll8(BLASLONG M, BLASLONG N, BLASLONG K,
                             ...
                             int32_t *C, BLASLONG ldc)
{
    ...
    rc = igemm_kernel_8x4_zvl256b_lmulmf2_unroll8(M, N, K, alpha, A_pack, B_pack, C, ldc);
    free(A_pack);
    free(B_pack);
    if (rc == 0) return 0;
    return scalar_gemm_full(M, N, K, alpha, A, B, C, ldc);
