        ...
        for (int i = 0; i < size; i++) {
            for (int k = 0; k < size; k++) {
                ...
                for (int j = 0; j < size; j += vl) {
                    vl = __riscv_vsetvl_e64m2(size - j);

                    // Load vector from B row k
                    vfloat64m2_t b_vec = __riscv_vle64_v_f64m2(&B[k * stride + j], vl);  //HERE stride, not size (but is right??)

                    // Broadcast scalar from A
                    vfloat64m2_t a_vec = __riscv_vfmv_v_f_f64m2(a_val, vl);

                    // Load current vector from C row i
                    vfloat64m2_t c_vec = __riscv_vle64_v_f64m2(&C[i * stride + j], vl);  //HERE stride, not size (but is right??)

                    // Multiply and accumulate
                    vfloat64m2_t prod = __riscv_vfmul_vv_f64m2(a_vec, b_vec, vl);
                    c_vec = __riscv_vfadd_vv_f64m2(c_vec, prod, vl);

                    // Store back to C
                    __riscv_vse64_v_f64m2(&C[i * stride + j], c_vec, vl);                //HERE stride, not size (but is right??)
                }
            }
        }
        return;
...
int main(int argc, char* argv[]) {
    ...
    printf("> BENCHMARK_RECORD : rvv2_smatmul_recursive %f, %d, %d\n", end_time - start_time, size, threshold);
